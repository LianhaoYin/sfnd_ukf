...
void UKF::Prediction(double delta_t) {
    ...
    /**
     * 1.1. augmentation of satae
     */
    
    // create augmented mean vector
    VectorXd x_aug = VectorXd(n_aug_); //  VectorXd x_aug = VectorXd(n_aug_); // yin
    ...
    for (int i = 0; i < 2 * n_aug_ + 1; ++i) {  // iterate over sigma points
        x_pre = x_pre + weights_(i) * Xsig_pred_.col(i);
    }
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
    ...
    Eigen::VectorXd z_pred_ = VectorXd(n_z);
    
    // measurement covariance matrix S
    Eigen::MatrixXd S_ = MatrixXd(n_z,n_z);
...
//void UKF::UpdateRadar(MeasurementPackage meas_package) { // yin
void UKF::UpdateRadar(MeasurementPackage meas_package) {
    ...
    MatrixXd R = MatrixXd(n_z,n_z);
    R.fill(0.0);
    //R(1,1) =  std_radr_*std_radr_;
    //R(2,2) = std_radphi_*std_radphi_;
    //R(3,3) = std_radrd_*std_radrd_;
    // yin
    ...
    //x_(1) =meas_package.raw_measurements_(1);//z_(1);
